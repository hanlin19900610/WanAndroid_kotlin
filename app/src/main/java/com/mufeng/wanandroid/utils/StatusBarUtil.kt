package com.mufeng.wanandroid.utils
...
object StatusBarUtil {
     ...
     * @return 半透明 View
     */
    private fun createTranslucentStatusBarView(activity: Activity, @androidx.annotation.IntRange(from = 0, to = 255) alpha: Int): View {
        // 绘制一个和状态栏一样高的矩形
        val statusBarView = View(activity)
        val params = LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, getStatusBarHeight(activity))
        statusBarView.layoutParams = params
     ...
     * @return 最终的状态栏颜色
     */
    private fun calculateStatusColor(@ColorInt color: Int, @androidx.annotation.IntRange(from = 0, to = 255) alpha: Int): Int {
        if (alpha == 0) {
            return color
        }
        val a = 1 - alpha / 255f
